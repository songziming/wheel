...
void fat_determine(uint8_t *sec0) {
    ...
    // 分析 FAT 类型
    if (clus_cnt < 4085) {
        log("[fat] this is FAT-12.\n");
    } else if (clus_cnt < 65525) {
        log("[fat] this is FAT-16.\n");
    } else {
        log("[fat] this is FAT-32.\n");
    }
}
...
int is_exfat(uint8_t *sec0) {
    ...
    if (0 != memcmp(bs->fsid, "EXFAT   ", 8)) {
        log("fs name field not valid.\n");
        return 0;
    }
    for (int i = 0; i < 53; ++i) {
        if (0 != bs->zero[i]) {
            log("BPB should all be zero.\n");
            return 0;
        }
    }
    ...
    if (0 != bs->vol_offset) {
        log("[exfat] warning: PartitionOffset not compliant.\n");
    }

    log("[exfat] fs create ok.\n");
    return 1;
}
