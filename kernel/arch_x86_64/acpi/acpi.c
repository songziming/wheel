...
static INIT_TEXT acpi_tbl_t *check_table(uint64_t addr) {
    acpi_tbl_t *tbl = (acpi_tbl_t*)(IDENTITY_MAP_ADDR + addr);
    if (bytes_sum(tbl, tbl->length)) {
        logk("warning: ACPI-%.4s checksum failed!\n", tbl->signature);
        return NULL;
    }

    pmrange_t *pmr = pmrange_at(addr);
...
static INIT_TEXT void parse_rsdp_v2(acpi_rsdp_t *rsdp) {
    if (bytes_sum(rsdp, rsdp->length)) {
        logk("warning: ACPI-RSDP v2.0 checksum failed!\n");
        return;
    }

    acpi_xsdt_t *xsdt = (acpi_xsdt_t*)rsdp->xsdt_addr;
    if (bytes_sum(xsdt, xsdt->header.length)) {
        logk("warning: ACPI-XSDT checksum failed!\n");
        return;
    }
...
INIT_TEXT void acpi_rsdp_parse(size_t addr) {
    ASSERT(0 != addr);
    ASSERT(0 == g_table_num);
    ASSERT(NULL == g_tables);

    acpi_rsdp_t *rsdp = (acpi_rsdp_t*)(IDENTITY_MAP_ADDR + addr);
    if (0 == rsdp->revision) {
        parse_rsdp_v1(rsdp);
    } else {
        parse_rsdp_v2(rsdp);
...
int acpi_table_count(const char sig[4]) {
    ...
    for (int i = 0; i < g_table_num; ++i) {
        ...
        if (0 == kmemcmp(sig, g_tables[i]->signature, 4)) {
            ++num;
        }
    }
    return num;
