...
static unsigned g_caret_row;        // 光标所在行号
static unsigned g_caret_col;        // 光标所在列号
...
INIT_TEXT void framebuf_init(uint32_t rows, uint32_t cols, uint32_t pitch, uint32_t addr) {
    ...
    g_caret_row = 0;
    g_caret_col = 0;
    // g_framebuf_color = 0xffffffff;

    framebuf_draw_caret(g_framebuf_color, g_caret_row, g_caret_col);
}
...
static void framebuf_putc(char ch) {
    ...
    if (g_caret_row >= g_em_rows) {
        ...
        kmemcpy(g_addr, g_back, g_rows * g_pitch);
        --g_caret_row;
        --r;
    }

    put_char_at(ch, g_framebuf_color, r, c);
}
