...
uint64_t pagetbl_create() {
    uint64_t  pa = (uint64_t)page_alloc() << PAGE_SHIFT;
    // uint64_t *va = (uint64_t *)(pa + DIRECT_MAP_ADDR);
    uint64_t *va = (uint64_t *)VIRT(pa);
    bset(va, 0, PAGE_SIZE);
    return pa;
}

void pml4_map(uint64_t *pml4, uint8_t idx) {
    //
}
...
uint64_t mmu_translate(uint64_t tbl, uint64_t va) {
    ...
    uint64_t *pd = (uint64_t *)((pdpe & MMU_ADDR) | DIRECT_MAP_BASE);
    ...
    uint64_t *pt = (uint64_t *)((pde & MMU_ADDR) | DIRECT_MAP_BASE);
