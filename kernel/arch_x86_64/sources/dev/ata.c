...
static INIT_TEXT void ata_detect(int secondary, int slave) {
    ...
    // klog("%s-%s io-base=%x, control-base=%x sel=%x\n",
    ...
    if ((0x14 == r4) && (0xeb == r5)) {
        ...
        klog("%s-%s unknown ATA device type %x-%x\n", name_channel, name_device, r4, r5);
        return;
    }
...
static void ata_show_info(blk_dev_t *blk) {
    ata_device_t *ata = containerof(blk, ata_device_t, blk);

    const char *channel = (ata->flags & ATA_SECONDARY) ? "secondary" : "primary";
    const char *device = (ata->flags & ATA_SLAVE) ? "slave" : "master";

    klog("%s, ata %s %s, sec_size=%d, sec_count=%ld\n",
        blk->name, channel, device, blk->sec_size, blk->sec_num);
...
void ata_pio_read_sector(blk_dev_t *blk, void *dst, uint64_t sector, uint64_t num) {
    ...
    // 如果出错，需要重置设备
    if (ata_wait_data(ch)) {
        klog("%s error when reading sector\n", ata->blk.name);
        ata_reset(ch);
        return;
    }
...
void ata_pio_write_sector(blk_dev_t *blk, const void *src, uint64_t sector, uint64_t num) {
    ...
    // 如果出错，需要重置设备
    if (ata_wait_data(ch)) {
        klog("%s error when writing sector\n", ata->blk.name);
        ata_reset(ch);
        return;
    }
...
INIT_TEXT void ata_pci_lib_init(const pci_dev_t *dev) {
    ...
    if (progif & 1) {
        ...
        klog("primary channel supports PCI-native mode\n");
        ch0_native = 1;
        progif |= 1;
    }
    ...
    if (progif & 4) {
        ...
        klog("secondary channel supports PCI-native mode\n");
        ch1_native = 1;
        progif |= 4;
    }
