...
void ata_pci_init(uint8_t bus, uint8_t slot, uint8_t func) {
    ...
    g_pci_write(bus, slot, func, 8, reg2);
    ...
    if (ch1_native) {
        ...
        uint32_t bar3 = g_pci_read(bus, slot, func, 0x1c);
        if (0 != bar3) {
            ASSERT(bar3 & 1);
    ...
    if (progif & (1 << 7)) {
        // klog("supports DMA!\n");
        uint32_t bar4 = g_pci_read(bus, slot, func, 0x20);
        if (bar4 & 1) {
    ...
    if (ch0_native || ch1_native) {
        uint32_t regf = g_pci_read(bus, slot, func, 0x3c);
