...
INIT_TEXT void framebuf_init(mb2_tag_framebuffer_t *fb) {
    ...
    g_backbuf = early_alloc(backsize);
    memset(g_backbuf, 0, backsize);
    ...
    // g_font = &g_font_terminux_32x16;
    g_font = &g_font_terminux_16x8;
    // g_font = &g_font_ubuntumono_16x8;
    ...
    // dbg_print("screen resolution w:%d h:%d, pitch:%ld\n", g_px_cols, g_px_rows, g_step);
    // dbg_print("font char size w:%d h:%d\n", g_font->width, g_font->height);
    // dbg_print("fake text resolution w:%d h:%d\n", g_em_cols, g_em_rows);
    // dbg_print("R:%08x, G:%08x B:%08x\n", g_px_r, g_px_g, g_px_b);
}
...
void framebuf_putc(char ch) {
    ...
    case '\r':
        ...
        break;
    default:
        framebuf_putc_at(ch, g_px_r | g_px_g | g_px_b, g_caret_row, g_caret_col);
        ++g_caret_col;
        break;
    }
    ...
    if (g_caret_row >= g_em_rows) {
        ...
        memcpy(g_backbuf, g_backbuf + line_size, (g_em_rows - 1) * line_size);
        memset(g_backbuf + (g_em_rows - 1) * line_size, 0, line_size);
        memcpy(g_addr, g_backbuf, g_px_rows * g_step);
        --g_caret_row;
    }
}
