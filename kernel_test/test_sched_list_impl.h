...
TEST(LIST_NAME, Diff) {
    ...
    EXPECT_EQ(0, LIST_CONTAINS(item1));
    EXPECT_EQ(&item2, LIST_HEAD());

    LIST_REMOVE(item3); // 2 5
    EXPECT_EQ(0, LIST_CONTAINS(item3));
    EXPECT_EQ(&item2, LIST_HEAD());
}
...
TEST(LIST_NAME, Same) {
    ...
    EXPECT_EQ(0, LIST_CONTAINS(item1c));
    EXPECT_EQ(&item1a, LIST_HEAD());

    LIST_REMOVE(item1a); // 1b 2a 2b 2c
    EXPECT_EQ(0, LIST_CONTAINS(item1a));
    EXPECT_EQ(&item1b, LIST_HEAD());

    LIST_REMOVE(item1b); // 2a 2b 2c
    EXPECT_EQ(0, LIST_CONTAINS(item1b));
    EXPECT_EQ(&item2a, LIST_HEAD());
}
